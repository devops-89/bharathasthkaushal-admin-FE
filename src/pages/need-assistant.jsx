...
import { Search, Filter, Eye, Clock, CheckCircle, XCircle, AlertCircle, X } from 'lucide-react';
...
const NeedAssistanceDashboard = () => {
  ...
  const fetchNeedAssistance = async () => {
    setLoading(true);
    try {
      const response = await needAssistanceControllers.getAllNeedAssistance(10, 1, statusFilter === 'ALL' ? '' : statusFilter);
      console.log("API Response:", JSON.stringify(response.data, null, 2));
      let tickets = response.data?.data?.docs || response.data?.docs || response.data || [];
      ...
      const mappedData = tickets.map((ticket, index) => {
        ...
        const userName = createdByUser?.name || 
                        `${createdByUser?.firstName || ''} ${createdByUser?.lastName || ''}`.trim() || 
                        (createdByUser?.email ? createdByUser.email.split("@")[0] : 'Unknown User');
        
        const userEmail = createdByUser?.email? createdByUser.email.split("@")[0] : 'Unknown User';
        ...
        return {
          ...
          issueDescription: ticket.issueDescription || 'No description provided',
  ...
  const handleViewDetails = async (ticket) => {
    try {
      const response = await needAssistanceControllers.getNeedAssistanceById(ticket.id);
      let fullTicket = response.data?.data || response.data || ticket.rawTicket || ticket;
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      await needAssistanceControllers.updateNeedAssistanceStatus(selectedTicket.id, updateData);
      setData(prev => 
        prev.map(item => 
          item.id === selectedTicket.id 
            ? { ...item, status: newStatus, adminRemarks: updateData.adminRemarks, updatedAt: new Date().toISOString() }
            : item
        )
      );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 p-6 ml-64 pt-20 flex-1">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold bg-gradient-to-r from-orange-500 to-orange-700 bg-clip-text text-transparent">Need Assistance</h1>
                ...
                <table className="w-full">
                  <thead className="bg-orange-600 text-white">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-semibold uppercase">Issue Type</th>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{selectedTicket.userId}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Issue Type</p>
                      <p className="font-medium text-orange-600">{getIssueTypeLabel(selectedTicket.issueType)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-700 bg-gray-50 p-4 rounded-lg">{selectedTicket.issueDescription}</p>
                </div>
                ...
                {/* Admin Remarks */}
                <div>
                  <label className="block text-sm font-semibold text-gray-900 mb-2">Admin Remarks</label>
                  <textarea
                ...
                {selectedTicket.adminRemarks && (
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-gray-900 mb-2 text-sm">Previous Admin Remarks</h4>
                    <p className="text-sm text-gray-700">{selectedTicket.adminRemarks}</p>
                  </div>
                )}
