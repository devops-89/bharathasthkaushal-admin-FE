...
const ProductDetails = () => {
  ...
  const handleCreateStepFormSubmit = async (e) => {
    ...
    try {
      ...
      // formData.append(
      //   "instructions",
      //   (createStepForm.instructions || "").trim(),
      // );
      if (createStepForm.materials && createStepForm.materials.trim() !== "") {
        formData.append("materials", createStepForm.materials.trim());
      }
      if (createStepForm.instructions && createStepForm.instructions.trim() !== "") {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 p-6 ml-64 pt-24">
                ...
                {product.admin_approval_status === "APPROVED" && (
                  <div className="space-y-3">
                    <button
                      ...
                      disabled={product?.build_status === "READY_FOR_AUCTION"}
                      className={`w-full px-4 py-4 ${product?.build_status === "READY_FOR_AUCTION" ? "bg-gray-300 cursor-not-allowed" : "bg-orange-600 hover:bg-orange-700"} text-white rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 shadow-lg ${product?.build_status === "READY_FOR_AUCTION" ? "" : "hover:shadow-xl"}`}
                    >
            ...
            <div className="lg:col-span-5 p-8 bg-gradient-to-br from-gray-50 to-white">
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-2 capitalize flex items-center gap-2">
                    {product?.product_name || "Unnamed Product"}
                  </h1>
                  {/*
                  {product.admin_approval_status !== "APPROVED" && (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {/* {product?.build_status && (
                        <>
                          ...
                          <span className={`px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-sm border transition-all ${product.build_status.toLowerCase() === "ready_for_auction"
                              ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                              : "bg-blue-50 text-blue-700 border-blue-200"
                            }`}>
                            <Layers className="w-4 h-4" />
                            {product.build_status.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}
                          </span>
                        </>
                      )} */}
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* Added By Section */}
                    {product?.addedBy && (
                      <div className="col-span-1 md:col-span-2 flex items-center gap-4 p-4 bg-indigo-50 rounded-xl hover:shadow-md transition-shadow border border-indigo-100">
                        <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center overflow-hidden border border-indigo-200 flex-shrink-0">
                          {product.addedBy.avatar ? (
                            <SecureImage
                              src={product.addedBy.avatar}
                              alt="Added By"
                              className="w-full h-full object-cover"
                            />
                          ) : (
