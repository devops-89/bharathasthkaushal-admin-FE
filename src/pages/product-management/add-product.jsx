...
const AddProduct = () => {
  ...
  const fetchCategories = async () => {
    try {
      const res = await categoryControllers.getCategory(1, 1000, "", true);
      setCategories(res.data?.data?.docs || []);
    } catch (error) {
    }
  };
  ...
  const preventNonIntegers = (e) => {
    // Allow necessary control keys
    const controlKeys = ["Backspace", "Delete", "ArrowLeft", "ArrowRight", "Tab", "Enter"];
    if (controlKeys.includes(e.key)) return;

    // Block everything that is not a number (0-9)
    if (!/^[0-9]$/.test(e.key)) {
      e.preventDefault();
    }
  };
  ...
  const handleInputChange = (e) => {
    ...
    // Strict positive integer validation for timeToMake
    if (name === "timeToMake" && typeof finalValue === "string") {
      finalValue = finalValue.replace(/\D/g, ""); // Remove any non-digit chars
      if (finalValue.startsWith("0")) {
        finalValue = finalValue.replace(/^0+/, ""); // Remove leading zeros
      }
    }
    ...
    try {
      ...
      const compressionPromises = files.map(async (file) => {
        try {
          ...
          // Use the type generated by the compressor (usually JPEG or WebP) for efficiency
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              {/* Image Previews */}
              {(images.length > 0 || compressingImagesCount > 0) && (
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {images.map((file, index) => (
                    <div key={index} className="relative group rounded-lg overflow-hidden border border-gray-200 aspect-square">
                      <div className="absolute top-2 left-2 bg-black/50 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow z-10">{index + 1}</div>
                      <img
                  ...
                  {/* Skeleton Loaders for images being compressed */}
                  {Array.from({ length: compressingImagesCount }).map((_, index) => (
                    <div key={`skeleton-${index}`} className="relative rounded-lg overflow-hidden border border-gray-200 aspect-square bg-gray-200 animate-pulse"></div>
                  ))}
                </div>
              )}
            </div>
